...
#Read data
df <- readRDS("./01_Wiki_00_Input/Relation_P31_P279_df.Rds")
colnames(df) <- c("Subject", "Property", "Object")
head(df)

if(!exists("Wikilabels")){ Wikilabels <- data.frame(readRDS("./01_Wiki_00_Input/WikiLab.Rds")) }
head(Wikilabels)

#view vignette
browseURL("https://github.com/kumeS/agGraphSearch/blob/main/vignettes/agGraphSearch-Wikidata-WF-New_v04.Rmd")
...
#表示用の変数
DataUp <- Data
colnames(DataUp) <- c("subject", "property", "parentClass", "depth")
...
######################################################################
#create dir
if(!dir.exists("01_Wiki_01_Out")){dir.create("01_Wiki_01_Out")}
######################################################################
head(Dat); dim(Dat)
...
d <- unique(c(Dat01[,c("subjectLabel")][grepl("wd:", Dat01[,c("subjectLabel")])], 
              Dat01[,c("parentClass")][grepl("wd:", Dat01[,c("parentClassLabel")])]))
length(d)
#[1] 139
...
##############################################
head(Dat01)
readr::write_excel_csv(Dat01, file = paste0("./01_Wiki_01_Out/", sub(".html", ".csv", FileName)), col_names = T)
#Dat01 <- data.frame(readr::read_csv(paste0("./01_Wiki_01_Out/", sub(".html", ".csv", FileName)), col_names = T))

##############################################
#グラフ末端のQIDエンティティ表記(ラベル無し)を削除する
##############################################
head(Dat01)
...
agVisNetwork(Graph=Dat01a,
             ...
             Output=TRUE,
             FilePath=FileName)
system(paste0("mv ", FileName, " ./01_Wiki_01_Out"))
...
head(DataUp)
head(Dat01a)
DataUp$triple <- paste0(DataUp$subject, ".", DataUp$property, ".", DataUp$parentClass)
Dat01a$triple <- paste0(Dat01a$subject, ".", Dat01a$property, ".", Dat01a$parentClass)
DataUp00 <- Dat01a[Dat01a$triple %in% unique(DataUp$triple),]

agVisNetwork(Graph=DataUp00,
             Selected=NULL, 
             Browse=TRUE, 
             Output=TRUE,
             ColoredSeed=QIDup,
             FilePath="DataUp.html")
system(paste0("mv ", "DataUp.html", " ./01_Wiki_01_Out"))
