...
MeshOthers_2021dfR <- rbind(MeshOthers_2021df.broaderDescriptor, MeshOthers_2021df.concept)
...
MeshLabel_2021df <- MeshLabel_2021df[MeshLabel_2021df$Property == "rdfs:label",]
head(MeshLabel_2021df)
dim(MeshLabel_2021df)
...
colnames(Dat) <- c("subject", "property", "parentClass", "OtherInfo", "subjectLabel", "parentClassLabel")
...
#Jpn
########################################################################
MeshLabels00 <- readRDS("./00_Mesh_Input/mesh2021_Label_en_df_R.Rds")
MeshLabels00 <- MeshLabels00[MeshLabels00$Property == "rdfs:label",]
...
if(ncol(DatJpn) == 4){colnames(DatJpn) <- c("subject", "property", "parentClass", "OtherInfo")}
DatJpn <- DatJpn[order(DatJpn$subject, decreasing = T),]
if(all(colnames(MeshLabels01) == c("Subject", "Object"))){
DatJpn00 <-  merge(DatJpn, MeshLabels01, by.x = "subject", by.y = "Subject", all = F, sort = F)
DatJpn01 <-  merge(DatJpn00, MeshLabels01, by.x = "parentClass", by.y = "Subject", all = F, sort = F)
colnames(DatJpn01)[c((ncol(DatJpn01)-1):ncol(DatJpn01))] <- c("subjectLabel", "parentClassLabel")
DatJpn02 <- DatJpn01[,c("subject", "property",
                  "parentClass", "OtherInfo",
                  "subjectLabel", "parentClassLabel")]
}

head(DatJpn02); dim(DatJpn02)
DatJpn02 <- Label2Jpn(Data=DatJpn02, Auth_Key="43725ffd-d3d3-a301-c92d-8d7e2070f71c:fx")
FileName <- paste0("agVisNetwork_polymers_Jpn_", format(Sys.time(), "%y%m%d-%H%M%S"),".html")

#All
agVisNetwork(Graph=DatJpn02,
             Selected=NULL, 
             Browse=FALSE, 
             Output=TRUE,
             FilePath=FileName)
